...
def extract_assistant_messages(data):
    ...
    return (
        assistant_contents[0] if assistant_contents else "Could not find any message..."
    )
...
def display_sidebar():
    with st.sidebar:
        ...
        st.markdown(
            "<h4 style='margin-bottom: 0px;'>Agents Online:</h4>",
            unsafe_allow_html=True,
        )
        ...
        if st.button(
            "✨ New Conversation ✨", key="new_conv_button", use_container_width=True
        ):
            start_new_conversation()

        # Simplified Conversation History
        st.markdown(
            "<h4 style='margin: 0px 0 0px 0;'>Recent Conversations:</h4>",
            unsafe_allow_html=True,
        )
        ...
        for idx, conv_dict in enumerate(st.session_state.conversations):
            ...
            title = (
                (first_user_message[:43] + "...")
                if len(first_user_message) > 43
        ...
        st.write("---")
        st.markdown(
            '<a href="/.auth/logout" target = "_self">Sign Out</a>',
            unsafe_allow_html=True,
        )
...
def display_chat():
    ...
    if st.session_state.current_conversation_index is None:
        st.write(
            "Please start a new conversation or select an existing one from the sidebar."
        )
        return

    question_options = []
    # Get the current conversation
    conversation_dict = st.session_state.conversations[
    ...
    question_options = [
        "Select a predefined question or type your own below"
    ...
    if selected_question != "Select a predefined question or type your own below":
        if (
            "last_selected_question" not in st.session_state
            or st.session_state.last_selected_question != selected_question
        ):
            st.session_state.last_selected_question = selected_question
            messages.append({"role": "user", "content": selected_question})
            with st.spinner(
                "Moneta agents are collaborating to find the best answer..."
            ):
                assistant_responses = send_message_to_backend(
                    selected_question, conversation_dict
                )
...
def start_new_conversation():
    st.session_state.conversations.append({"messages": [], "name": "New Conversation"})
    st.session_state.current_conversation_index = (
        len(st.session_state.conversations) - 1
    )
    st.session_state.last_selected_question = (
        "Select a predefined question or type your own below"
    )

    # reset choice of dropdown
    st.session_state["question_selectbox"] = (
        "Select a predefined question or type your own below"
    )
...
def main():
    ...
    if principal:
        ...
        st.session_state.display_name = next(
            (claim["val"] for claim in claims if claim["typ"] == "name"), "Default User"
        )
    else:
