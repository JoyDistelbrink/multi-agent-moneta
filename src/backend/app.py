...
async def http_trigger(request_body: dict = Body(...)):
    ...
    if load_history is not True and not user_message:
        raise HTTPException(
            status_code=400, detail="<message> is required when not loading history!"
        )
    ...
    with tracer.start_as_current_span(session_id):
        ...
        if usecase_type == "fsi_insurance":
            ...
            raise HTTPException(
                status_code=400, detail="Use case not recognized/not implemented..."
            )
        ...
        handler_type = os.getenv(
            "HANDLER_TYPE", "semantickernel"
        )  # Expected values: "vanilla", "semantickernel"
        ...
        if load_history is True:
            ...
            return JSONResponse(content=conversation_list, status_code=200)
        ...
        return JSONResponse(
            content={"chat_id": chat_id, "reply": new_messages}, status_code=200
        )
