...
class TicketingOrchestrator(SemanticOrchastrator):
    def __init__(self):
        super().__init__()
...
""",
    ...
    def create_agent_group_chat(self):
        ...
        helper_agent = self.create_agent(
            service_id="gpt-4o",
            kernel=self.kernel,
            definition_file_path="sk/agents/ticketing/helper.yaml",
        )
        responder_agent = self.create_agent(
            service_id="gpt-4o",
            kernel=self.kernel,
            definition_file_path="sk/agents/ticketing/responder.yaml",
        )

        agents = [helper_agent, responder_agent]
        ...
        agent_group_chat = AgentGroupChat(
            ...
            termination_strategy=self.create_termination_strategy(
                agents=[responder_agent, helper_agent],
