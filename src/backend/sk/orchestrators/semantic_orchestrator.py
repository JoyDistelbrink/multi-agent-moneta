...
class SemanticOrchastrator:
    def __init__(self):
        ...
        deployment_namem4oini = "gpt-4o-mini"
        deployment_nameo1mini = "o1-mini"
        ...
        self.gpto1mini_service = AzureAIInferenceChatCompletion(
            ai_model_id="gpt-o1-mini",
            client=aio_inference.ChatCompletionsClient(
                endpoint=f"{str(endpoint).strip('/')}/openai/deployments/{deployment_nameo1mini}",
                credential=aio_identity.DefaultAzureCredential(),
                credential_scopes=["https://cognitiveservices.azure.com/.default"],
            ),
        )

        self.gpt4omini_service = AzureAIInferenceChatCompletion(
            ai_model_id="gpt-4o-mini",
            client=aio_inference.ChatCompletionsClient(
                endpoint=f"{str(endpoint).strip('/')}/openai/deployments/{deployment_namem4oini}",
                credential=aio_identity.DefaultAzureCredential(),
                credential_scopes=["https://cognitiveservices.azure.com/.default"],
            ),
        )
    ...
    async def process_conversation(self, user_id, conversation_messages):
        ...
        chat_history = [
            ...
            for d in filter(
                lambda m: m["role"] in ("system", "developer", "assistant", "user"),
        ...
        response = list(
            reversed([item async for item in agent_group_chat.get_chat_messages()])
        )
